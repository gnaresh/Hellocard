...
var m = Math,
	...
	iScroll = function (el, options) {
		...
		that.options = {
			...
			//fix TapHighlight desactivation
			fakeTapHighlightClass: "iscroll-tap-highlight",
			fakeTapHighlightTimeout : 500
		};
...
iScroll.prototype = {
	...
	//FIX HIGHLIGHTTAP DISABLING
	_showTapHighlight: function(targetElement) {
		var that = this,
        tapHighlightClass = that.options.fakeTapHighlightClass, regex = new RegExp('(\\s|^)' + tapHighlightClass + '(\\s|$)');

        if (!targetElement.className.match(regex)) {
        targetElement.className += " " + tapHighlightClass;
        setTimeout(function() { targetElement.className = targetElement.className.replace(regex, '');
		}, 
		(typeof that.options.fakeTapHighlightTimeout == "number") ?	that.options.fakeTapHighlightTimeout : 500);
		}
	}, 
				...
				if (that.doubleTapTimer && that.options.zoom) {
					...
					that.doubleTapTimer = setTimeout(function () {
						...
						if (target.tagName != 'SELECT' && target.tagName != 'INPUT' && target.tagName != 'TEXTAREA') {
							that._showTapHighlight(target); // FIX TAP HIGHLIGHT
